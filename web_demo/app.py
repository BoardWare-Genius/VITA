...
async def chat(request: Request):
    ...
    vision_tag, audio_tag = False,False
    if vis_file and vis_file.filename: vision_tag = True
    if audio_file and audio_file.filename: audio_tag = True
    ...
    if vision_tag: 
        if is_image(vis_file.filename) or is_video(vis_file.filename):
            ...
            history, task_history = add_video(history, task_history, filepath)
        else:
            return JSONResponse(content={"Error": "The Vision file is not supported"}, status_code=status.HTTP_400_BAD_REQUEST)
    # print("type of audio {}".format(type(audio_file)))
    # print('whether audio is None {}'.format(bool(audio_file)))
    # print('audio is  ###{}###'.format(audio_file))
    if (query and not audio_tag) or (not query and audio_tag):
        if query:         ## Text submit       
            history, task_history = add_text(history, task_history, query)
        else:             ## Audio submit
            audio_filepath = await _save_file(audio_file)
            history, task_history = add_audio(history, task_history, audio_filepath)
    else:
        # if audio_file:
        #     print('whether audio is None {}'.format(bool(audio_file)))
        #     print('audio file is  #######{}###'.format(bool(audio_file.file)))
        #     print('audio file len is  #######{}###'.format(audio_file.content_type))
        #     print('audio filename #####{}######'.format(bool(audio_file.filename)))
        # if not audio_file:
        #     print('see audio is None {}'.format(not audio_file))
    ...
    # history = predict(history, task_history)
    # audio_output = stream_audio_output(history, task_history)
    return StreamingResponse(predict(history, task_history))
    # return StreamingResponse(stream_audio_output(predict(history, task_history)))
...
if __name__ == '__main__':
    ...
    uvicorn.run(app="web_demo.app:app", host="0.0.0.0", port=7860, log_level="debug",reload=True)
