    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            for (param_name, weight_name, shard_id) in stacked_params_mapping:
                ...
                weight_loader = getattr(param, "weight_loader",
                                        default_weight_loader)
                ## Modify to input and output for embedding and lm.head to align tokenizer vocab size -- enlarge
                if name in ('language_model.embed_tokens.weight','lm_head.weight'):
                    print('{} {}:{}'.format('#'*30,name, loaded_weight.shape))
                    padding = torch.zeros((self.config.vocab_size - loaded_weight.shape[0], loaded_weight.shape[1]), dtype=loaded_weight.dtype, device=loaded_weight.device)
                    loaded_weight = torch.cat([loaded_weight,padding])
                    print('{} {}:{}'.format('#'*30,name, loaded_weight.shape))
                # weight_loader(param, loaded_weight)
                try: 
                    weight_loader(param, loaded_weight)
