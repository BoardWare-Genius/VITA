...
async def chat(request: Request):
    ...
    def predict(task_history):
        ...
        async def streaming_resp():
            ...
            async for item in result_generator:
                current_text=item.outputs[0].text
                newly_generated_text = current_text[len(las_output_text):]
                # if text in PUNCTUATION:
                #     continue
                if first_time:
                    print("{}TTFT for VITA vllm: {} {}".format('-'*15,time.time() - start_time,'-'*15))
                    first_time = False
                if newly_generated_text:
                    # print(newly_generated_text)
                    las_output_text = current_text
    ...
    async def collect_results_stream_tts(results_generator):
        history_generated_text = ''
        las_output_text = ''
        sample_rate = 24000
        async for newly_generated_text in results_generator:
            # TODO: collect every token from generator, split and clean the sentence, push single sentece toward tts and then yield the buffer          
            puncts = [",", "，", ".", "。", "?", "\n", "？", "!", "！", "、"]
            if any(p in newly_generated_text for p in puncts):
                ...
                sentence = replace_equation(remove_special_characters(history_generated_text))
                embeddings = llm_embedding(torch.tensor(tokenizer.encode(sentence)).to(device))
                for seg in tts.run(embeddings.reshape(-1, 896).unsqueeze(0), decoder_topk,
                                    None, 
                                    codec_chunk_size, codec_padding_size):
                    # if idx == 0:
    ...
    if text_return:
        return EventSourceResponse(predict(task_history))
        # return JSONResponse(content={'response':predict(task_history)})

    return EventSourceResponse(collect_results_stream_tts(predict(task_history)),media_type='audio/wav')
...
def load_model(model_path = './demo_VITA_ckpt/'):
    ...
    llm = AsyncLLMEngine.from_engine_args(vllm_engine_args)
    # llm = LLM(
    #     model=model_path,
    #     dtype="float16",
    #     tensor_parallel_size=1,
...
if __name__ == '__main__':
    ...
    uvicorn.run(app="web_demo.app_stream:app", host="0.0.0.0", port=7860, log_level="debug",reload=True,reload_dirs='./web_demo',reload_includes='.py')
