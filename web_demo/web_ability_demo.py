...
def _launch_demo(llm, model_config, sampling_params, tokenizer, feature_extractor, tts, llm_embedding,lora_request1):
    def predict(_chatbot, task_history):
        ...
        output = llm.generate(datapromt, sampling_params=sampling_params,lora_request = lora_request1)
...
def parse_args():
    ...
    parser.add_argument("--enable_lora", type=bool, default=False,help='Whether to load lora adapter for inference')
    parser.add_argument("--adapter_path", type=str, default=None,help='Path to lora adapter')
    args = parser.parse_args()
    return args

def main(model_path,enable_lora = False, adapter_path = None):
    ...
    lora_request1 = LoRARequest("self_adapter_v1", 1, lora_local_path=adapter_path) if adapter_path else None
    ...
    _launch_demo(llm, model_config, sampling_params, tokenizer, feature_extractor, tts, llm_embedding,lora_request1)


if __name__ == '__main__':
    args = parse_args()
    main(args.model_path, enable_lora=args.enable_lora, adapter_path= args.adapter_path)
